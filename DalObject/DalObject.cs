...
namespace DL
{
    sealed class DalObject : IDL
    {
        ...
        {
            if (DataSource.ListLines.Where(x => x.Active == true).FirstOrDefault(x => x.ID == lineID) != null)
                DataSource.ListLines.Where(x => x.Active == true).FirstOrDefault(x => x.ID == lineID).Active = false;
            else
        ...
        {
            if (DataSource.ListAdjacentStations.FirstOrDefault(x => (x.Station1 == stationCode1 && x.Station2 == stationCode2)) != null)
                return DataSource.ListAdjacentStations.FirstOrDefault(x => (x.Station1 == stationCode1 && x.Station2 == stationCode2));
            else
        ...
        public void DeleteAdjacentStation(AdjacentStations adjStat)
        {
            ...
            if (DataSource.ListAdjacentStations.FirstOrDefault(x => x.Station1 == adjStat.Station1 && x.Station2 == adjStat.Station2) != null)
                DataSource.ListAdjacentStations.RemoveAll(x => x.Station1 == adjStat.Station1 && x.Station2 == adjStat.Station2);
            else
                throw new ArgumentException("Invalid AdjacentStations Entity");
        }
        #endregion
        ...
        {
            DO.LineTrip oldLineTrip = DataSource.ListLineTrips.FirstOrDefault(x => x.LineID == newLineTrip.LineID);
            newLineTrip.Active = true;
            if (oldLineTrip == null)
                DataSource.ListLineTrips.Add(newLineTrip);
            else if (oldLineTrip.Active == false)
            {
                oldLineTrip.Active = true;
                newLineTrip.CopyPropertiesTo(oldLineTrip);
            }
            else
                throw new InvalidOperationException("ERROR! that Line Trip already exists!");
        }
        ...
        {
            if (DataSource.ListLineTrips.Where(x => x.Active == true).FirstOrDefault(x => x.LineID == lineID) != null)
                return DataSource.ListLineTrips.Where(x => x.Active == true).FirstOrDefault(x => x.LineID == lineID);
            else
                return null;
        }
        ...
        {
            if (DataSource.ListLineTrips.Where(x => x.Active == true).FirstOrDefault(x => x.LineID == lineID) != null)
                DataSource.ListLineTrips.Where(x => x.Active == true).FirstOrDefault(x => x.LineID == lineID).Active = false;
            else
                throw new ArgumentException("Invalid Line ID");
        }
        #endregion
